...
- 'tabelas_utilizadas' - Criação e inserção dos dados da tabela 'estoque', 'categorias' e 'cores', utilizadas em todos os exercícios
- 'joins_introducao.sql' - INNER JOIN, LEFT JOIN, filtros com WHERE
- 'joins_agregacao.sql' - GROUP BY com JOIN, HAVING, estatísticas
- 'joins_subconsultas.sql' - Subconsultas correlacionadas com JOIN 
- 'tabelas_derivadas.sql' - Subconsultas no FROM
